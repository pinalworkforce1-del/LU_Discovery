...
function bind(v:HTMLVideoElement|null){
  if(!v||v.dataset.luRail)return;
  v.dataset.luRail="1";
  ["play","pause","ended","volumechange","loadedmetadata"].forEach(e=>v.addEventListener(e,queue));
}
...
function refresh(){
  ...
  if(!rail)rail=makeRail();
  if(rail.parentElement!==shell||rail.previousElementSibling!==stage)stage.insertAdjacentElement("afterend",rail);
  ...
  const replay=rail.querySelector<HTMLButtonElement>('[data-lu="replay"]')!;
  ...
  audio.disabled=!has;
  audio.setAttribute("aria-pressed",String(Boolean(v?.muted)));
  ...
  next.hidden=!source;
  next.disabled=!source||source.disabled;
}
